...
my @expression = (127, int(127 * 0.9), int(127 * 0.8), int(127 * 0.7),
                  int(127 * 0.6), int(127 * 0.5), int(127 * 0.4), int(127 * 0.3),
		  int(127 * 0.2), int(127 * 0.1)
);
# ----------------------------------------------------------------------------
...
{
    my ($track, $start, $chan, $expression) = @_;
    printf STDOUT "%d, %d, Control_c, %d, 11, %d\n", $track, $start, $chan, $expression;
...
for (my $i=0; $i < scalar(@instruments); $i = $i + 1)
{
	...
	for my $e (@expression)
	{
	    print_expression($track, $start, $chan, $e);
	    $start = $start + $NOTELTH;
	}
	print_note_stop($track, $stop, $chan, $note);
	$start = $start + $NOTELTH;
	$stop = $stop + $NOTELTH * $NUM_EXPR;
    }
    print_track_end($track, $ENDTRACKS);

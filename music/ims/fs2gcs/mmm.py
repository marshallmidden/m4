...
def notes_fitted_into_measures():
    print_stderr('\nNOTDONEYET - notes_fitted_into_measures')
    pass
...
def separate_args(wline, integers, nospaces, original_line):
    def b4it(array_args, b4, integers, nospaces, original_line):
        global staff_name
        ...
        if b4 == '':
            ...
            if b4 in staff_name:
                if staff_name[b4] is None:
                    assign_another_voice(b4, 'separate_args', original_line)
                # fi
    ...
    while w < len(strng):
        ...
        # fi
        b4 = s[0:x]
        array_args = b4it(array_args, b4, integers, nospaces, original_line)
        b4 = ''
        w = w + x + 1
    # elihw
    ti = integers
    array_args = b4it(array_args, b4, ti, nospaces, original_line)
    return array_args
...
def do_clef(t_args, original_line):
    ...
    vargs = get_voice_others(t_args, 1,2, 'clef', original_line)
    if vargs is None:
        return
    # fi

    if len(vargs) == 1:
        wargs = separate_args(vargs[0], False, True, original_line)
...
# ----------------------------------------------------------------------------
def get_voice_others(t_args, mn, mx, command, original_line):
    if t_args is None or t_args == '':
        print_error("{} command does not have any arguments".format(command), original_line)
        return None
    # fi
    ...
    if len(vargs) < mn or len(vargs) > mx:
        print_error("{} command does not have correct number of arguments (mn..{}) verses {}".format(mn,mx,len(vargs)), original_line)
        return None
    # fi
    return vargs
# End of get_voice_others
...
def do_key(t_args, original_line):
    ...
    vargs = get_voice_others(t_args, 1,3, 'key', original_line)
    if vargs is None:
        return
    # fi
    ...
    if vs is None:                          # One argument, all voices change.
        for ve in range(1, len(volume)+1):
        ...
        for ve in range(1, MAXVOICES+1):
            set_staff_arr_double(ve, k, measure_on, key_voice, None, original_line)
        # rof
        return
    ...
    for w in wargs:
        ...
        if mlth[f1][measure_on] != 0:
            ...
            return
        # fi
        set_staff_arr_double(f1, k, measure_on, key_voice, None, original_line)
    # rof
    return
# End of do_key
...
def do_instrument(t_args, original_line):
    ...
    # Note: 99 allows any number ... instrument with spaces within.
    vargs = get_voice_others(t_args, 1, 99, 'instrument', original_line)
    if vargs is None:
        return
    # fi
    if len(vargs) == 1:
        INST = " ".join(vargs).lower()
        vargs = ["1..{}".format(len(volume))]
    else:
    ...
    # fi
    if foundinstrument is None:
        print_error("do_instrument - midi_instruments does not have INST='{}'".format(INST), original_line)
...
def do_middle_c(t_args, original_line):
    ...
    if note_to_decode[0] == '(':
        ...
        if note_to_decode <= 0 or note_to_decode >= 127:
            ...
            return
        # fi
        compute_middle_c(round(thenote))
        return
     # fi
...
def do_page(t_args, original_line):
    ...
    if mesg is not None or n is None:
        if page_warn_once:
            if not args.nooncewarning:
                print_warning("ONCE#A: page is also a variable, and cannot set character string variable to it.", original_line)
            # fi
...
def do_pan(t_args, original_line):
    ...
    vargs = get_voice_others(t_args, 1,2, 'pan', original_line)
    if vargs is None:
        return
    # fi
    if len(vargs) == 1:
...
def do_reverb(t_args, original_line):
    ...
    vargs = get_voice_others(t_args, 1,2, 'reverb', original_line)
    if vargs is None:
        return
    # fi
    if len(vargs) == 1:
    ...
    if type_error is not None:
        print_error("reverb error - {} ... r='{}'".format(type_error,r), original_line)
        return
    # fi
    if f1 is None:
    ...
    for w in wargs:
        f1, type_error = is_float(w, is_float_number, False)
        if type_error is not None:
            print_error("reverb error - {} ... f1='{}'".format(type_error,f1), original_line)
            return
        # fi
        if f1 is None:
            print_error("voice {} is not a number.".format(w), original_line)
...
def do_xpose(t_args, original_line):
    ...
    if len(vargs) > 1:
        ...
        if kl.lower() in key_sig:
            ...
            x = str(round(f1)) # Must be an integer as a string..
        # fi
        vs = '{}..{}'.format(1, len(volume))    # all voices
    # fi
    if type(kl) is str:
        if kl not in key_sig:
...
def readhintfile():
    ...
    while True:                                         # Read the file loop.
        ...
        # fi
        if args.lines:
            print_stderr("linecount={} line='{}'".format(linecount,line))
        # fi
        line = line.strip()
        if line == '':                                  # Blank line
            continue

 ...
 * 
 * used by midi2abc and abc2midi
 *
 * modified 11/6/96 by James Allwright
 * all unsigned longs changed to long
 * mf_sec2ticks modified to work using PCC compiler
 * mf_write_midi_event, size changed from long to int
 * mf_write_meta_event, size changed from long to int,
 *   type changed from char to int.
 * mf_write_track_chunk changed to expect Mf_writetrack
 *   to return number of divisions till end of track
 * perror changed to mferror
 * PCCFIX and NOFTELL #ifdefs added.
 * modified 12/8/96 
 * mf_sec2ticks routine corrected.
 * private functions declared 'static' at start of file to stop gcc
 * generating error messages.
 * modified 23/6/98 mferror made more helpful.
 * 30/9/98 NOFTELL fixes cleaned up
...
char *data;
{
    ...
    if(chan > 63) {
      mferror("error: MIDI channel greater than 64");
    };

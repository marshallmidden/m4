#!/bin/sh
/usr/bin/echo "/usr/lib/dracut/modules.d/95resume/parse-resume.sh $*" > /dev/kmsg
...
if ! getarg noresume; then
    if [ -n "$resume" ]; then
        ...
        {
/usr/bin/echo "/usr/lib/dracut/modules.d/95resume/parse-resume.sh creating initqueue --finished $a_splash $resume" > /dev/kmsg

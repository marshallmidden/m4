...
/*
Need a routine to test all of raid 5 parity 9 (8+1) read possibilties.
...
c) read blocks 3 sector at a time increasing by 1 sector.
	...
Max read is 2mb, which is 2097152, or 4096 sectors.
*/
...
int main()
{
    ...
    int count;
    ssize_t red;
    unsigned long long *rb = (unsigned long long *)buffer;
    ...
    {
      fprintf(stderr, "\nreading %llu sectors at block offset:\n", (unsigned long long)sectors);
	...
	for (i = 0; i < count; i++)
	{
	    red = read(fd, buffer, sectors * SECTOR_SIZE);
	    if (red < 0)
	    {
		perror("read:");
	    ...
	    for (j = 0; j < sectors; j++)
	    {
	    	if (rb[j * LL_BS] != (unsigned long long)(lba + i*sectors + j))
		{
		    fprintf(stderr, "\nDid not get %llu, got %llu for %d,%d -- sectors=%llu, lba=%llu, count=%d\n", (unsigned long long)(lba + i*sectors + j), rb[j * LL_BS], j,i, (unsigned long long)sectors, (unsigned long long)lba, count);
		    exit(4);
		}
	    }
	}

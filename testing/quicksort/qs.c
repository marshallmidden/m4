...
static unsigned long long _start_tsc_;
static unsigned long long _end_tsc_;

#define START_TIMING	{ gettimeofday(&_start_time_, NULL); \
			_start_tsc_ = get_tsc();}
#define STOP_TIMING	{ _end_tsc_ = get_tsc(); \
...
static void quickSort(int a[], int lo, int hi)
{
	...
	case -1: break;
	case 0:  break;
	case 1:  if (a[lo] > a[p - 1]) { swap2(a[lo], a[p - 1]); }; break;
	case 2:  sort3(a[lo], a[lo + 1], a[p - 1]); break;
	...
	case -1: break;
	case 0:  break;
	case 1:  if (a[p + 1] > a[hi]) { swap2(a[p + 1], a[hi]); }; break;
	case 2:  sort3(a[p + 1], a[p + 2], a[hi]); break;
...
/* . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . */
    ...
    unsigned long long qs_tsc = 0;
    unsigned long long qsort_tsc = 0;
...
// printarray("\nUnsorted array", orig, 0, MAX_ARRAY - 1);
    ...
    qs_tsc += (_end_tsc_ - _start_tsc_);

    START_TIMING;
                                        qsort(b, MAX_ARRAY, sizeof(int), compare);
    STOP_TIMING;
    timeradd(&qs_time, &_limited_, &qsort_time);
    qsort_tsc += (_end_tsc_ - _start_tsc_);
...
printf("\n"); fflush(stdout);
    printf("quickSort - Time used %d.%06d  tsc=%llu\n", (int)qs_time.tv_sec, (int)qs_time.tv_usec, qs_tsc);
    printf("    qsort - Time used %d.%06d  tsc=%llu\n", (int)qsort_time.tv_sec, (int)qsort_time.tv_usec, qsort_tsc);
#endif /* STATIC_ALL */
/* . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . */
#ifdef STATIC_RANDOM
    init_random();

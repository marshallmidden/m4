...
# See file SETUPrunoffset

# This modification goes thru increasing the offset from 1 (passed in) to lth-1.
#
# The idea is to have 19 vdisks created, associated with dot14 in disks /dev/sdc ... sdv.
...
#   Do a write, read for each vdisk.
#   Wait for I/O to stop to dot14 (3 in a row, one second apart).
#   Check all raids not degraded, and operational.
#   Loop:
#     fail chosen pdisk.
...
sub do_pdisk_fail_test
{
    ...
    chomp($dt);
    printf STDERR "$dt DONE with test $test_number/$r5check_offset_blocks.\n";

    #   Loop:
    foreach my $pdisk ( @pdisks )
    {
        $test_number++;
    printf STDERR "Starting to fail PDISK $pdisk, test $test_number/$r5check_offset_blocks\n";
...
my $dt = `/bin/date '+%Y-%m-%d@%H:%M:%S'`;
chomp($dt);
printf STDERR "$dt starting program runoffsetr5check.perl\n";

my $test_number = 1;

for ( ; $r5check_offset_blocks < ($r5check_contiguous_blocks - 1); $r5check_offset_blocks++)
{
    do_pdisk_fail_test();
}
...
##############################################################################
# End of File runoffsetr5check.perl
#
## Modelines:

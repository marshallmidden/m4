...
$Id: Olddrivecodeload.pl 126999 2010-02-09 21:40:48Z tom_marlin $
...
#        ($cfgFile) = shift @ARGV;
        ...
        logWarning("Drive/Bay Code Script Started");
...
#
    ...
    if ( GOOD != $ret )
    {
        logWarning( "Drive/Bay Code Failed:  Error during update Reason Code: $ret ");
        #return (ERROR);
    }
...
# end change
        ...
        for ( $j = 0; $j < scalar(@FWList); $j++ )
        {
            ...
            if ( $drvmatch == 1 )
            {
                ...
                if ( $codeMatch != 1 )
                {
                    logWarning (" Drive/Bay Code on Device: $j with firmware: $FWList[$j]"); 
                    $ret = SendCode2Drive( $ctlr, $id, $fnList[$j],
                                $wwnLo, $wwnHi, $lun ) ;
                    if ( $ret != GOOD )
                    {
                         # First Attempt failed.  Try again
                         logWarning( "  First Attempt failed.  Sleeping 10 seconds and try again...");
                         sleep 10;
                         ...
                         if ( $ret != GOOD )
                         {
                              logWarning( ">>>>>>>> Failure updating code on device $j <<<<<<<<");
                              $flag = ERROR;
                              #return ERROR;
                         }
    ...
    if ( $flag == ERROR )
    {
        ...
        logWarning ("Error:  some devices were not updated. Please Contact Service");
        #return ERROR;
    }
...
# is now:
    ...
    printf "  Writing %s to %8.8x%8.8x \n", $filename, $wwnLo, $wwnHi;
     ...
     if ($rsp{STATUS} != PI_GOOD)
     {
         logWarning (">>>>>>>> Error sending code to device <<<<<<<<");
         PrintError(%rsp);
         return ERROR;
     }

     logWarning ("Drive/Bay Code Upgrade Complete");

...
sub ProcessFidBucket($\@$$$)
{
    ...
    for (my $i = 0; $i < @$pFidArray; $i++)
    {
        ...
        if ($pData and length($$pData) > 0)
        {
            ...
            {
                $rc = CCBEDecodeFids ($$pData, $fid , ">$bucketName");
                if ($rc != GOOD)
                {
    ...
    if ($procLog)
    {
	    ...
	    {
		$logSim = "$progDir/../CCB/LogSim/Src/logsim";
		if (-x "$logSim")
		{
		    $foundLogsim = 1;
		}
...
my $version = q$Revision: 126999 $;
print 
"---------------------------------------\n".
"             $script\n".
"Copyright 2000-2005 XIOtech Corporation\n".
...
while(@ARGV)
{
    ...
    if ($argType eq "ipaddress")
    {
        ...
        if (!defined($port))
        {
            # If a port number was not passed on the command line default to 3100
            $port = 3100;
        }
...
#
# $Log$
# Revision 1.7  2006/10/05 14:13:46  MiddenM
# TBolt00016149 - SnapDump.pl was forgotten in files needed to get fid 312.
#
# Revision 1.6  2006/07/17 20:38:31  RustadM
# TBolt00014770
# Move 750 branch onto main.
#
# Revision 1.3.24.1  2006/02/24 14:17:23  MiddenM
#
# Merge from WOOKIEE_EGGS_GA_BR into MODEL750_BR
#
# Revision 1.5  2006/01/17 13:08:51  ChannappaS
# Adding I_TGD info to NVRAM Part 5
#
# Revision 1.4  2006/01/12 11:05:27  ChannappaS
# Adding ISCSI and GeoRaid data to SnapDump
#
# Revision 1.3  2005/06/03 21:25:35  HoltyB
# TBolt00000000: Changes for the case of an ICON snapshot.
#
# Revision 1.2  2005/05/06 16:01:44  RysavyR
# TBolt00012834: break out the Linux fids into separate files. Rev by Mike Mc.
#
# Revision 1.1.1.1  2005/05/04 18:53:54  RysavyR
# import CT1_BR to shared/Wookiee
#
# Revision 1.20  2005/04/06 16:13:07  McmasterM
# TBolt00012691: Snapdump error: Can't remove temporary directory
# Tweak the file so that help is available without prompt.  Other changes to
# address this defect went into the fmtFids.pm module.
#
# Revision 1.19  2005/02/24 13:43:34  RysavyR
# TBolt00012347: Added fid 353 so that it gets grabbed and decoded.
#
# Revision 1.18  2005/02/23 14:57:03  RysavyR
# TBolt00012347: Numerous fixes to SnapDump and underlying CCBE library to clean up errors seen on the snap server.
#
# Revision 1.17  2005/02/14 17:08:01  RysavyR
# TBolt00012264: Look at $buildTime instead of the script name to determine if execuatble is running or not. Rev by Craig M.
#
# Revision 1.16  2005/02/11 22:00:59  RysavyR
# TBolt00012264: Added a build time to the SnapDump.exe banner page so that we can rebuild from that time (in CVS) if need be.  Also, a message in SnapDump.pl that directs users to the Test/Executables directory to run SnapDump.exe instead of the .pl version. Rev by Craig M.
#
# Revision 1.15  2005/02/04 20:24:36  RysavyR
# TBolt00012244: Perform additional range checking when decoding timestamps.  Also pick up everything else up to this point (including new Wookiee stuff).
#
# Revision 1.14  2005/01/26 17:09:13  HoltyB
# TBolt00012008:  Changes to support Application logs on Wookiee.
# Reviewed by Mike McMaster.
#
# Revision 1.13  2004/12/08 21:52:19  HoltyB
# TBolt00011907:  Added Changes to support Linux file transfers.
#
# Revision 1.12  2004/10/22 19:30:47  RysavyR
# TBolt00000000: Many good updates, including checking for logsim at startup,
# saving log binary files in case logsim fails, plus other cleanup.
#
# Revision 1.11  2004/08/16 20:03:40  RysavyR
# TBolt00011053: Fix the FID 280 (vdisk cache) decoder. Also added NAME to VDISK display output.
#
# Revision 1.10  2004/07/08 15:12:46  RysavyR
...
#
# Revision 1.9  2004/02/24 22:38:13  RysavyR
# Added return error codes for Tony. An RC of 0 is good, as always.
# The associated exe also contains the new heap decoder stuff.
#
# Revision 1.8  2004/02/16 22:49:54  RysavyR
...
#
# Revision 1.7  2004/02/13 17:31:21  RysavyR

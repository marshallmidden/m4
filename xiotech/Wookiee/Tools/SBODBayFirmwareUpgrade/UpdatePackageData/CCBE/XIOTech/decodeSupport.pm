...
$Id: decodeSupport.pm 28577 2007-07-31 20:03:50Z m4 $
...
sub FmtIRAM
{
 ...
 #   $info{QB_QPOS1}  = $item2;
    ...
    #
    #        .align  4                       # Quadword aligned structures
...
#
# !!! NOTE: IF THIS STRUCTURE CHANGES, EMC mrp.h MUST ALSO CHAGE !!!!!!!!!
#                                                                  *****
...
sub FmtDataString
{
    ...
    for ($i=0; $i<$length; $i+=16) 
    {
        ...
        {
            $str .= sprintf($printfTpl2, $rowData[$j]);
        }
...
#    $msg .= sprintf("                    rsvd1:  0x%08x  \n",$item2);
    ...
    $fmt = sprintf("x%d LL CCS L  ",$offset);      # generate the format string
    ($item1, $item2, $item3, $item4, $item5, $item6) =  
                        unpack $fmt , $$bufferPtr;
...
#    $msg .= sprintf("rsvd2:  0x%02x 0x%04x 0x%08x  \n", $item4, $item5, $item6);

    $offset += 16;                         #   bytes processed
    $address += 16;                  


    #        UINT8       rsvd2[199];                 /* RESERVED   (192 left)                    */
    #
#    $msg .= "\n 192 additional bytes rsvd2 \n";
#    $msg .= FmtDataString( $bufferPtr, $address, "word", 192, $offset);
    
    $offset += 192;                         #   bytes processed
    $address += 192;                  
...
##############################################################################
# $Log$
# Revision 1.6  2006/09/28 10:27:24  BharadwajS
# TBolt00015876 Correcting IRAM decoding
#
# Revision 1.5  2006/07/17 20:38:32  RustadM
# TBolt00014770
# Move 750 branch onto main.
#
# Revision 1.1.1.1.28.4  2006/04/26 07:35:42  BharadwajS
# Adding error codes for new commands added
#
# Revision 1.1.1.1.28.3  2006/04/17 06:07:40  BharadwajS
# Log Acknowledgement
#
# Revision 1.1.1.1.28.2  2006/04/12 09:51:17  GudipudiK
# packet interface changes for CCBE changes
#
# Revision 1.1.1.1.28.1  2006/02/24 14:17:24  MiddenM
#
# Merge from WOOKIEE_EGGS_GA_BR into MODEL750_BR
#
# Revision 1.4  2006/01/24 04:43:12  BharadwajS
# Removing unnecessary printf
#
# Revision 1.3  2006/01/17 13:08:51  ChannappaS
# Adding I_TGD info to NVRAM Part 5
#
# Revision 1.2  2006/01/12 11:05:28  ChannappaS
# Adding ISCSI and GeoRaid data to SnapDump
#
# Revision 1.1.1.1  2005/05/04 18:53:54  RysavyR
# import CT1_BR to shared/Wookiee
#
# Revision 1.24  2005/04/01 12:37:34  BalemarthyS
# TBolt00012254 - FE NVRAM Part-5 SnapDump currectly decoded and removed new line characters
#
# Revision 1.22  2005/03/03 11:26:59  BalemarthyS
# TBolt00012254 - FE NVRAM Part5 SnapDump - Proper indentation
#
# Revision 1.21  2005/03/02 15:50:23  BalemarthyS
# TBolt00012254 - FE NVRAM Part 5 SnapDump FID related changes
#
# Revision 1.20  2005/02/04 20:18:23  RysavyR
# TBolt00012244: Perform additional range checking when decoding timestamps.
#
# Revision 1.19  2004/12/14 21:31:20  RysavyR
...
#
# Revision 1.18  2004/07/08 15:12:45  RysavyR
...
#
# Revision 1.17  2004/06/28 16:21:00  MenningC
# tbolt00000000: fix for error in fid 291. reviewed by Al.
#
# Revision 1.16  2004/06/01 20:49:42  RysavyR
# TBolt00000000: Fixed up Ethernet Statsistics for HyperNode - FID 303.
#
# Revision 1.15  2004/02/24 22:32:37  RysavyR
# TBolt00010172: Add heap stats / top users to CCB NVRAM backtrace
# (to track down memory leaks).  Rev by Tim S.
#
# Revision 1.14  2004/02/16 22:48:49  RysavyR
# TBolt00000000: Updated and reworked how trace data is decoded.  One common
# routine now does it all.  Split out log files by type. Tunnel'd events are now decoded
# as to their base Packet command.
#
# Revision 1.13  2004/02/02 21:03:53  RysavyR
# TBolt00009987: Added support for FID 303 -- CCB Statistics.
#
# Revision 1.12  2004/01/28 17:02:22  RysavyR
# Fixed "Uptime" calculation in CCB NVRAM decoder.
#
# Revision 1.11  2003/12/16 19:09:49  RysavyR
# Clean up CR/LF's in string output.
#
# Revision 1.10  2003/12/12 16:09:28  SchibillaM
# TBolt00009629: Add decoders for cache maps and cache device paths.  Fix bug
# which resulted in an extra (invalid) GeoPool entry.
#
# Revision 1.9  2003/12/10 18:02:07  SchibillaM
...
#
# Revision 1.8  2003/12/09 16:35:12  SchibillaM
# TBolt00009629: Change timestamp decode in FmtCCBETSnap.
#
# Revision 1.7  2003/12/08 20:23:26  SchibillaM
# TBolt00009629: Initial decoder for CCB NVRAM Flash Copies (FID 298).
#
# Revision 1.6  2003/11/24 18:50:44  SchibillaM
# TBolt00009629: Change formatting of register output.
#
# Revision 1.5  2003/11/21 20:38:24  SchibillaM
# TBolt00009629: FmtCCBNVR1FIDNew() replaces FmtCCBNVR1FID().  Lots of
# formatting changes to the CCB NVRAM FID.
#
# Revision 1.4  2003/11/10 22:06:07  McmasterM
# TBolt00000000: Change 'pdisks loop' command to display port letter
#
# Revision 1.3  2003/06/25 13:13:29  MenningC
# TBOLT00000000: support for fid 291; reviewed by Mark S and Eric T
#
# Revision 1.2  2003/06/16 21:56:35  MenningC
# TBOLT00000000: fixed IRAM formatting; reviewed by Steve H.
#
# Revision 1.1  2003/06/16 19:09:13  MenningC
# TBOLT00000000: import ddrdecode formatters into ccbcl; Reviewed by Randy and Erc
#
# Revision 1.4  2003/05/21 21:35:10  MenningC
# Tbolt00000000:more FIDs added to decoders; reveiwed by JW
#
# Revision 1.3  2003/05/14 20:52:43  MenningC
# Tbolt00000000:fix a typo
#
...
#
# Revision 1.20  2003/05/07 15:59:34  MenningC
# Tbolt00000000: added some of  the protected area on diag code; reviewed by Olga
#
# Revision 1.19  2003/05/01 20:23:36  MenningC
# Tbolt00000000; spelling fix
#
# Revision 1.18  2003/05/01 19:18:30  MenningC
# Tbolt00000000; updates for nvram part 2, reviewed by JW
#
# Revision 1.17  2003/04/30 19:13:51  MenningC
# tbolt00000000:fixes to decoding of VLARs. Reviewed by Olga
#
# Revision 1.16  2003/04/04 17:05:59  MenningC
# TBOLT00000000; $day to $date in firmware header; Found and reveiwed by Brett.
#
# Revision 1.15  2003/03/26 21:30:58  WerningJ
# Removed calls to unused modules
# Reviewed by Craig M
#
# Revision 1.14  2003/03/21 15:57:38  MenningC
# added record length validation - reviewed by JW
#
# Revision 1.13  2003/03/20 21:43:50  MenningC
# initial c/i for record length validation - not reviewed
#
# Revision 1.12  2003/03/18 16:00:46  MenningC
# updates for  part 1.  reveiwed by JW
#
# Revision 1.11  2003/03/14 19:56:23  MenningC
# support for nvram part 1, reveiwed by JW
#
# Revision 1.10  2003/03/07 14:02:11  MenningC
# tbolt00000000 enhancement for JT, reviewed by JT
#
# Revision 1.9  2003/03/06 14:13:51  MenningC
# tbolt00000000 enhancement for JW, reviewed by the other JW
#
# Revision 1.8  2003/03/05 20:08:08  MenningC
# tbolt00000000 updates to run under Linux, reviewed by JW
#
# Revision 1.7  2003/03/04 21:43:39  McmasterM
# TBolt00007557: Incorrect backtrace data on ECC error
# Corrected some code paths that resulted in some of the backtrace data being
# overwritten when it shouldn't have been.  Without this fix, the backtrace data
# could present readings taken from two separate backtrace captures (confusing!).
# Changes will be reviewed by Kendall after checkin.
#
# Revision 1.6  2003/03/04 17:41:09  MenningC
# tbolt00000000; fixed nvr record 17, reviewed by J Williams
#
# Revision 1.5  2003/02/26 16:05:56  MenningC
# Tbolt00000000 updates for the constants record, reviewed by J Werning
#
# Revision 1.4  2003/02/14 16:10:17  MenningC
# fix some nvram stuff; reveiwed by Brett
#
# Revision 1.3  2003/02/10 22:58:13  MenningC
# Added more decode rings
#
# Revision 1.2  2003/02/08 16:17:04  MenningC
# Added more decode rings
#
# Revision 1.1  2003/02/07 17:38:50  MenningC
# new file
#
#
#

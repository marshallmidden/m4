...
$Id: decodeRings.pm 29877 2007-08-20 18:04:57Z m4 $
...
BEGIN {
    ...
    #$VERSION = do { my @r = (q$Revision: 29877 $ =~ /\d+/g); sprintf "%d."."%02d" x $#r, @r }; # must be all one line, for MakeMaker
...
#        .set    fh_targetID,fh_productID+4      # Target ID (EMC/FE/BE, Boot/Diag/Proc) <w>
#                                                #                                *****
...
#    $msg .= sprintf("reserved: 0x%02x \n", $item8);
    ...
    $msg .= sprintf("Start Offset of Local Images   0x%08X\n", $item1);
#    $msg .= sprintf("reserved                       0x%08x \n", $item2);
...
#                    $offset, $address, $length;
        ...
        {
#            printf "GeoPool START   offset=0x%X  address=0x%X  length=0x%X\n",
#                    $offset, $address, $length;
        
            $msg .= "--- Begin GeoPool record structure ----------------------------\n";

            #    typedef struct NVRG
            #    {
            #        DEF_GEOPOOL_INFO geoPool;
            #    } NVRG;
            #    
            #    typedef struct DEF_GEOPOOL_INFO
            #    {
            #        UINT16      primaryControllerBitmap;
            #        DEF_GEOPOOL geoPools[DEF_GP_MAX_POOLS];
            #    } DEF_GEOPOOL_INFO;
            #    
            #    typedef struct DEF_GEOPOOL
            #    {
            #        UINT16      controllerBitmap;
            #        UINT16      bayBitmap;
            #        UINT8       name[DEF_GP_NAME_SIZE];
            #    } DEF_GEOPOOL;
            #    
            #    #define DEF_GP_MAX_POOLS    4                                    
            #    #define DEF_GP_NAME_SIZE   16
            # 
            $fmt = sprintf("x%d a2",$offset);      # generate the format string
            ($item1) = unpack $fmt , $$bufferPtr;
            
            my @primaryControllerBitmap = XIOTech::cmdMgr::ParseBitmap($item1);
            $msg .= sprintf("Primary controller bitmap: @primaryControllerBitmap\n");
            ...
            # Variable to hold the size of a GeoPool struct as defined above.
            # Use this to determine the number of GeoPools.
            my $sizeOfGeoPool = 20;
            
            # Length of the entire GeoPool record, as read from the header.
            # The actual data length may be less but this will allow us to
            # calculate the correct starting point for the next record type.
            # Subtract 2 bytes for the primaryControllerBitmap which was
            # processed above.
            my $geoPoolRecordLength = $rec_length - 4 - 2;
             
            # Ofset used within the GeoPool data. 
            my $geoPoolOffset = $offset; 
            
            # Calculate number of entries based on record size an accounting
            # for the primaryControllerBitmap.
            my $numEntries = int($geoPoolRecordLength / $sizeOfGeoPool);
            
#            printf "geoPoolRecordLength=$geoPoolRecordLength    numEntries=$numEntries\n";

            for ($i = 0; $i < $numEntries; $i++)
            {
                $fmt = sprintf("x%d a2",$geoPoolOffset);      # generate the format string
                ($item1) = unpack $fmt , $$bufferPtr;
                
                my @controllerBitmap = XIOTech::cmdMgr::ParseBitmap($item1);
                $msg .= sprintf("Controller bitmap: @controllerBitmap    ");
                $geoPoolOffset += 2;
                
                $fmt = sprintf("x%d a2",$geoPoolOffset);      # generate the format string
                ($item1) = unpack $fmt , $$bufferPtr;
                
                my @bayBitmap = XIOTech::cmdMgr::ParseBitmap($item1);
                $msg .= sprintf("Bay bitmap: @bayBitmap    ");
                $geoPoolOffset += 2;
            
                $item1 = FmtString( $bufferPtr, $geoPoolOffset, 16 );
                $msg .= sprintf("GeoPool name: $item1\n");
                $geoPoolOffset += 16;
            }
            ...
            # the header.  This may differ from the actual amount of data 
            # processed.
            $offset  += $geoPoolRecordLength;
            $address += $geoPoolRecordLength;
            $length  -= $geoPoolRecordLength;
            
#            printf "GeoPool END     offset=0x%X  address=0x%X  length=0x%X\n",
#                    $offset, $address, $length;
        }
        elsif ($rec_type == 0x16)
        {

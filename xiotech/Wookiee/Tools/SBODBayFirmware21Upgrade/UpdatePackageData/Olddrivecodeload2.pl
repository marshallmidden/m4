...
$Id: Olddrivecodeload2.pl 149462 2010-10-22 19:32:41Z tom_marlin $
...
# end change
    ...
    for $i (0..$#{$rsp{LIST}})
    {
        print "Examining drive  " . $rsp{LIST}[$i] . "\n";
        $id =  $rsp{LIST}[$i];
...
# end change
        ...
        print  "  PD_PRODID:     $info{PS_PRODID}\n";
        ...
        for ( $j = 0; $j < scalar(@FWList); $j++ )
        {
            ...
            if ( $drvmatch == 1 )
            {
                print "  Device ID matched, index = $j, old rev is $rev, needs to be $FWList[$j] \n";
                ...
                if ( $codeMatch != 1 )
                {
                    ...
                    if ( $ret != GOOD )
                    {
                        print ">>>>>>>> Failure updating code on device $j <<<<<<<<\n";
                        $flag = ERROR;
                        #return ERROR;
                    }
...
# is now:
    ...
    printf "  Writing %s to %8.8x%8.8x \n", $filename, $wwnLo, $wwnHi;
     ...
     if ($rsp{STATUS} != PI_GOOD)
     {
         print ">>>>>>>> Error sending code to device <<<<<<<<\n";
         PrintError(%rsp);
         return ERROR;
     }

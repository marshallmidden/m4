...
extern struct memctrl_dev_attribute attr_log_ue;
extern struct memctrl_dev_attribute attr_log_ce;
extern struct memctrl_dev_attribute attr_panic_on_ue;

/*************************************************************
 * On Fully Buffered DIMM modules, this help function is
 * called to process UE events
 */
static void edac_mc_handle_fbd_ue(struct mem_ctl_info *mci,
                        unsigned int csrow,
                        ...
                        unsigned int channelb, char *msg)
{
        ...
        if (*attr_log_ue.value)
                edac_mc_printk(mci, KERN_EMERG,
                        "UE row %d, channel-a= %d channel-b= %d "
                        "labels \"%s\": %s\n", csrow, channela, channelb,
                        labels, msg);

        if (*attr_panic_on_ue.value)
 ...
 */
static void edac_mc_handle_fbd_ce(struct mem_ctl_info *mci, 
...
{
	//const struct i5400_dev_info *i5400_dev = &i5400_devs[dev_idx];
	struct i5400_pvt *pvt;
	struct pci_dev *pdev;
...
static void i5400_stop_and_scream(struct pci_dev *pdev)
{
	...
	for (dev = 0; dev < 10; ++dev) {
		if (pdev) {
			...
			i5400_printk(KERN_WARNING,
				"MC: Stop and scream for device %d is %d\n",
				dev, value & 1);

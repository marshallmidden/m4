...
static int lm93_detect(struct i2c_adapter *adapter, int address, int kind)
{
	...
	if (!data) {
		//dev_dbg(&new_client->dev, "out of memory!\n");
		err = -ENOMEM;
		goto err_out;
	}
	...
	if ((LM93_SMBUS_FUNC_FULL & func) == LM93_SMBUS_FUNC_FULL &&
			!disable_block) {
		dev_dbg(&new_client->dev, "using SMBus block data transactions\n");
		update = lm93_update_client_full;
	} else if ((LM93_SMBUS_FUNC_MIN & func) == LM93_SMBUS_FUNC_MIN) {
		dev_dbg(&new_client->dev, "disabled SMBus block data "
			"transactions\n");
		update = lm93_update_client_min;
	} else {
		dev_dbg(&new_client->dev, "detect failed, "
	...
	strlcpy(new_client->name, "lm93", I2C_NAME_SIZE);
	...
	/* Tell the i2c subsystem that a new client has arrived */
	...
	err = sysfs_create_group(&new_client->dev.kobj, &lm93_attr_grp);
	if (err)
		goto err_detach;
...
err_remove:
	dev_err(&new_client->dev, "error registering hwmon device.\n");
	sysfs_remove_group(&new_client->dev.kobj, &lm93_attr_grp);

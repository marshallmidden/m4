...
void delete_from_lists(struct qelem *thread)
{
    int i=0;
    struct qelem *tmp;
    tmp = actor_list.q_forw;
    while (tmp != &actor_list)
    {
        if(tmp == thread)
        {
            log_debug(3,"actor_list %d deleting thread %lx\n",i++,(long)thread );
            remque(tmp);
            tmp = actor_list.q_forw;
        }else
        {
    ...
    tmp = pend_list.q_forw;
    while (tmp != &pend_list)
    {
        if(tmp == thread)
        {
            log_debug(3,"pend_list %d deleting thread %lx\n",i++,(long)thread );
            remque(tmp);
            tmp = pend_list.q_forw;
        }else
        {
    ...
    tmp = poll_list.q_forw;
    while (tmp != &poll_list)
    {
        if(tmp == thread)
        {
            log_debug(3,"poll_list %d deleting thread %lx\n",i++,(long)thread );
            remque(tmp);
            tmp = poll_list.q_forw;
         }else
         {
...
actor_delete(actor_t *thread)
{
	...
	case ACTOR_POLL_WAITING:
		log_debug(4, "deleting a scheduled/waiting thread!");
		remque(&thread->item);
		break;
	default:
		break;

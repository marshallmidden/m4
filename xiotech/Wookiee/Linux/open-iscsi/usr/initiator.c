...
int
session_free_task(iscsi_session_t *session);
extern void delete_from_lists(struct qelem *thread);
char sysfs_file[PATH_MAX];
static int
session_find_hostno(uint32_t sid);

int find_sg_device(uint32_t sid)
{
	char filename[256];
	DIR *dir;
	struct dirent *entry;
    ...
    host_no = session_find_hostno(sid);

    if(host_no == -1)
    {
        log_error("find_sg_device: no host attached with session %d\n",sid);
        return -1;
    }
	sprintf(filename,"/sys/class/iscsi_session/session%d/"
			"device/target%d:0:0/%d:0:0:0",sid,host_no,host_no);
	dir = opendir(filename) ;
	if(dir == NULL)
	{
		log_debug(4,"The file %s does not exist\n",filename);
		return -1;
	}
	...
	do {
		...
		if (pid > 0) {
			...
			while (!(rc = waitpid(pid, &status, WNOHANG)) &&
				...
				sleep(1);
			if (!rc)
				log_debug(2, "could not delete device %s "
					  "after delay\n", sysfs_file);
		}
		close(fd);
...
__session_scan_host(iscsi_session_t *session, queue_task_t *qtask)
{
	...
	if (pid == 0) {
		...
		close(fd);
        /*
         ** find the sg device and fill it in response structure
         */
        qtask->u.login.rsp.u.session.transport_handle = session->transport_handle;
        qtask->u.login.rsp.u.session.session_id = session->id;
        sprintf(qtask->u.login.rsp.u.session.device_name,"sg%d", find_sg_device(session->id));

		write_mgmt_rsp(qtask, MGMT_IPC_OK);

		log_debug(4, "scanning host%d completed\n", hostno);
        log_warning("session%d:%d host no. %d is operational between i_name = %s t_name = %s tag = %d device = %s",
                session->id, session->conn[0].id,hostno,session->initiator_name,session->target_name,
                session->portal_group_tag,qtask->u.login.rsp.u.session.device_name);
...
__session_conn_create(iscsi_session_t *session, int cid)
{
	...
	log_debug(3, "session %p noop_out interval %u timeout %u\n", 
		  session, conn->noop_out_interval, conn->noop_out_timeout);
...
__send_pdu_timedout(void *data)
{
	...
	if (conn->send_pdu_in_progress) {
		...
		log_debug(2, "send_pdu timer timedout!");
	}
}
...
session_put(iscsi_session_t *session)
{
	...
	if (session->refcount == 0) {
        ...
        delete_from_lists(&(session->mainloop.item));
		free(session);
	}
}
...
__session_create(node_rec_t *rec, iscsi_provider_t *provider)
{
    ...
    session->failed = 0;
    /*
    ** session id is initialized to -1, instead of zero because
    ** kernel returns session id(s) from zero.
    */
    session->id = -1;
	strcpy(session->initiator_name,dconfig->initiator_name );
...
__session_conn_queue_flush(iscsi_conn_t *conn)
{
	...
	for (i = 0; i < count; i++) {
        ...
        /*
           * put the recvpool pointer back in pool
           * if the event type is RECV_PDU
           */
        if(item->event_type == EV_CONN_RECV_PDU)
        { 
            recvpool_put((iscsi_conn_t *)(item->context),(void*)*(uintptr_t*)queue_item_data(item)); 
        }    
		/* do nothing */
				 ...
				 int conn_cleanup)
{
	...
	if (conn_cleanup)
	    ...
	    session->provider->utransport->ep_disconnect(&(session->conn[0]));
		session_conn_destroy(session, conn->id);
		if (conn->id == 0)
			__session_destroy(session);
	}
...
print_param_value(enum iscsi_param param, void *value)
{
	...
	case ISCSI_PARAM_PERSISTENT_ADDRESS:
		log_debug(3, "set operational parameter %d to: %s", param, (char *)value);
		break;
	default:
...
__session_conn_recv_pdu(queue_item_t *item)
{
	...
	if (conn->state == STATE_IN_LOGIN) {
		...
		log_debug(1, "ignoring incomming PDU in XPT_WAIT. "
	        ...
	        recvpool_put((void*)conn, (void*)conn->recv_handle);
		return;
...
__session_conn_poll(queue_item_t *item)
{
    ...
    if(session->failed == 1)
    {
		...
		actor_delete(&conn->connect_timer);
        log_debug(1,"forcing cleanup session i_name %s t_name %s tag %d\n",session->initiator_name,
                    session->target_name,session->portal_group_tag); 
        goto cleanup;
    }
	if (conn->state != STATE_XPT_WAIT)
		return;
...
__conn_error_handle(iscsi_session_t *session, iscsi_conn_t *conn)
{
    ...
    strcpy(msg2proc.initiator_name,session->initiator_name);
    strcpy(msg2proc.target_name,session->target_name);
    msg2proc.pgt = session->portal_group_tag;
    msg2proc.event_type = ASYNC_CLOSE_SESSION;
	int i;
	...
	case STATE_LOGGED_IN:
        ...
        /*we don't support reopening of session it should be directed from PROC*/
	...
	session_conn_cleanup(conn);
/*
** Now the connection is stopped, we shall send a message to PROC 
*/
        log_debug(1,"seding a message to PROC - closed session %d for i_name = %s t_name = %s tag = %d\n",id,
               msg2proc.initiator_name,msg2proc.target_name,msg2proc.pgt);
       send_msg2_proc(&msg2proc);
}
...
iscsi_session_t*
session_find_by_rec(node_rec_t *rec,char *initiator_name)
{
	iscsi_session_t *session;
	struct qelem *item;
	int i;
	...
	for (i = 0; i < num_providers; i++) {
		...
		while (item != &provider[i].sessions) {
			...
			log_debug(6, "looking for session with rec_id [%06x]...",
				  session->nrec.id);
			if (rec->id == session->nrec.id && !strcmp(session->initiator_name,initiator_name)) {
				return session;
			}
			item = item->q_forw;
		}
...
session_login_task(node_rec_t *rec, queue_task_t *qtask)
{
	...
	if (!session)
        ...
        if(qtask->u.login.req.command == X_MGMT_IPC_SESSION_LOGIN)
        {
        /*
        ** add initiator name in session structure
        ** session_create does not take any argument
        */
           strcpy(session->initiator_name, qtask->u.login.req.u.o_session.initiator_name);
 
        }else if(qtask->u.login.req.command == X_MGMT_IPC_DISCOVERY )
        {
           strcpy(session->initiator_name, (char *)qtask->u.login.req.u.discovery.initiator_name);
        }
	...
	if (rc < 0 && errno != EINPROGRESS) {
		...
		log_error("cannot make a connection to %s:%s (%d)",
			 conn->host, serv, errno);
        /* close the socket fd before destroying the session */
	    session->provider->utransport->ep_disconnect(conn);
	...
	qtask->u.login.rsp.command = qtask->u.login.req.command;
...
session_logout_task(iscsi_session_t *session, queue_task_t *qtask)
{
	...
	if (conn->state == STATE_XPT_WAIT &&
        ...
        if(session->failed != 1)
        {
            ...
            queue_produce(session->queue, EV_CONN_POLL, qtask, 0, NULL);
            actor_schedule(&session->mainloop);
            log_debug(1, "logout request in connecting state for session i_name %s  t_name %s tag %d\n",
                    session->initiator_name,session->target_name,session->portal_group_tag);
        }
        goto send_response;
	}
...
session_free_task(iscsi_session_t *session)
{
    ...
    return session_conn_cleanup(conn);

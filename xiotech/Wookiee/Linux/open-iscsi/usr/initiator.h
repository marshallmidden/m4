...
#define RECVPOOL_MAX   8	
    ...
    int      logout_mgmt_fd;    /* this socket fd is used to send delayed response of a logout request for a 
                                *  session which is in XPT_WAIT state*/
} iscsi_conn_t;
...
typedef struct iscsi_session {
	...
	char *target_alias;
	char initiator_name[TARGET_NAME_MAXLEN + 1];
...
extern iscsi_session_t* session_find_by_rec(node_rec_t *rec,char *initiator_name);

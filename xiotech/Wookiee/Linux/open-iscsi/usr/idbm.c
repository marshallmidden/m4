...
{	
	if (db->refs > 0) {
		db->refs++;
        log_error("database reference %d \n",db->refs);
		return 0;
	}
	...
	if ((db->discdb = idbm_open(DISCOVERY_FILE,
        ...
        log_error("database discovery not opened");
		return -1;
	}
	
	if ((db->nodedb = idbm_open(NODE_FILE, access(NODE_FILE, F_OK) != 0 ?
				    O_CREAT|O_RDWR : O_RDWR)) == NULL) {
        log_error("database node not opened");
...
idbm_discovery_setup_defaults(discovery_rec_t *rec, discovery_type_e type)
{
	...
	if (type == DISCOVERY_TYPE_SENDTARGETS) {
		...
		rec->u.sendtargets.conn_timeo.login_timeout=2;
...
idbm_sync_config(idbm_t *db)
{
	...
	if (!f) {
		log_debug(5, "cannot open configuration file %s",
				db->configfile);
		return;
	}
...
/*
* The function finds the node given target name and portal group tag
* record id is returned in case of success otherwise it returns -1
* 
*/
int idbm_find_node(idbm_t *db, char *target_name,unsigned short pgtag)
{
	datum key, data;
	DBM *dbm ;
    node_rec_t ret_node;
	...
	for (key=dbm_firstkey(dbm); key.dptr != NULL; key=dbm_nextkey(dbm)) {
		...
		if (idbm_dbversion_check(rec->dbversion))
		{
			log_debug(7,"dbversion check failed\n");
			exit(-1);
		}

		if(!strcmp(rec->name,target_name) && pgtag == rec->tpgt)
		{
			ret_node = *rec;
			id = idbm_uniq_id(key.dptr);
			log_debug(7,"idbm_find_node found the node with target name %s, pgtag=%d\n",target_name,pgtag);
			break;
...
discovery_rec_t*
x_idbm_new_discovery(idbm_t *db, char *ip, int port,
			discovery_type_e type, char *info,  iscsiadm_rsp_t *rsp)
{
	char *ptr, *newinfo;
	discovery_rec_t *drec;
	node_rec_t *nrec;
		...
		if ((dp = strchr(ptr, '='))) {
			*dp = '\0'; dp++;
			if (!strcmp(ptr, "DTN") || !strcmp(ptr, "TN")) {
				...
				{
				   /*
				   ** node added. now add it in the response to proc
				   */
				   strcpy(rsp->u.targetlist.target[count].name,nrec->name);
				   inet_pton(AF_INET,nrec->conn[0].address,&(rsp->u.targetlist.target[count].ip));
				   rsp->u.targetlist.target[count].port = nrec->conn[0].port;
				   rsp->u.targetlist.target[count].pgtag = nrec->tpgt;
				   count++;
					log_error("node added count = %d\n",count);
				}
			} else {
				log_error("can not parse discovery info value. "
					  "Bug?");
	...
	rsp->u.targetlist.cnt = count;
	rsp->err  = MGMT_IPC_OK;
	log_error("final node count = %d\n",count);

...
# DATE:         5/12/2001
#
# DESCRIPTION:  Annotate a "PCB Walk/Stack Unwind" file with associated
#               function names.
#
#====================================================================
...
while(<F>) {
    ...
    if(/ +0x[0-9a-f]{16} {16}/) {
        @line = split;
        push @unsortedFuncs, "$line[0],$line[1],$curModule";
    }
}
close F;

@sortedFuncs = sort @unsortedFuncs;

for ($i = 0; $i < scalar(@sortedFuncs); ++$i)
{
    @line = split /,/, $sortedFuncs[$i];
    push @funcs, [oct $line[0], $line[1], $line[2]];
}
